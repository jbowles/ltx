...
\title[Pragmatic Logic] % (optional, use only with long paper titles)
{Towards a Pragmatic Logic of Arguments\\ \small (with Special Reference to Computational Intelligence)}

\subtitle
{An Informal Talk for Sci-Tech Undergrads} % (optional)

\author[Joshua Bowles] % (optional, use only with lots of authors)
{Joshua Bowles\inst{1}$^,$\inst{2}}
% - Use the \inst{?} command only if the authors have different
%   affiliation.

\institute[Utah Valley University] % (optional, but mostly needed)
{
  \inst{1}%
...
\date[Science and Technical Writing] % (optional)
{\today / Lecture}
